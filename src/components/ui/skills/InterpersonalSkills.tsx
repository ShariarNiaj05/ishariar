...
const InterpersonalSkills = async () => {
  ...
  const { interpersonalSkills } = skillsResponse as SkillsResponse;

  const content =
    Array.isArray(interpersonalSkills) && interpersonalSkills.length > 0
      ? interpersonalSkills.map((skill: Skills) => ({
